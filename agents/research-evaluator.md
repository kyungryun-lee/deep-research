...
- **Adaptive thinking**: Opus 4.7은 복잡 판단 시 thinking을 자동 engage합니다. 명시적 budget 설정 불필요.
- **Tokenizer 변동**: 같은 텍스트가 Opus 4.6 대비 1.0-1.35× 토큰. 차원당 응답을 2문장 이하로 압축 (CHG-074 토큰 예산 규칙 준수).
- **temperature/top_p/top_k 명시 금지**: Opus 4.7은 이 파라미터 명시 시 400 에러. 기본값(자동) 사용.
...
등급 기준: `SOURCE_TIERS.md` 참조 (Worker와 동일한 단일 정의 사용).
...
#### 코드 위임 차원 (AI 채점 생략 가능)
- **structure_coherence / structure**: `dr-score structure`가 100% 코드로 계산. 당신은 논리적 흐름(coherence)만 추가 판단하되, 점수는 코드 값이 덮어씁니다.
- **coverage**: `dr-score sea`가 SEA 충족률로 계산. 당신은 질적 커버리지만 판단하되, 점수는 코드 값이 덮어씁니다.

→ 이 2개 차원은 **간략히 평가** (각 1문장)하고 나머지 차원에 집중하세요.

#### 토큰 예산 규칙
각 차원의 rationale은 **2문장 이하**로 작성합니다.
"왜 이 점수인가"를 핵심만 — 장문 서술은 토큰 낭비입니다.

#### Hard Floor 규칙
Accuracy < 40점이면 반드시 feedback에 **[Hard Floor 경고]** 태그를 포함합니다.
이 경우 `dr-score calc`가 총점을 50점으로 상한합니다.
...
### Evidence Ratio (코드 사전계산 — dr-score xref)
`code_metrics.xref_density`가 이미 `claims_with_evidence / total_claims`를 계산합니다.
- xref_density ≥ 0.9 → Accuracy에 +5 보너스
- 0.7-0.9 → 보통
- 0.7 미만 → Accuracy에 -10 감점
**AI가 직접 계산하지 않습니다** — 코드 계산값을 참고하여 채점에 반영만 합니다.

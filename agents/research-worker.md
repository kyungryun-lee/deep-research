...
- `focus_area`: 집중 영역
- `anchor_sources`: (선택) 이전 리서치에서 검증된 핵심 소스 URL 목록
- `core_facts`: (선택) 이전에 검증된 핵심 팩트 (반증 없이 변경 금지)
- `phase_3a_summary`: (심화 검색 시) 이전 단계 결과 요약
...
### 0. 앵커 소스 재확인 (anchor_sources가 있을 때)
anchor_sources가 제공된 경우 **검색 전에** 앵커 URL을 WebFetch로 재확인합니다.
- 앵커 소스가 여전히 접근 가능하고 내용이 유효한지 확인
- 앵커 소스에서 업데이트된 정보가 있으면 기록
- core_facts와 앵커 소스 내용이 모순되면 **명시적으로 [반증 발견] 태그** 부착
- 앵커 재확인 후, 나머지 쿼리는 앵커가 커버하지 않는 영역에 집중
...
- core_facts가 있으면 이와 모순되는 정보에 **[Core 충돌]** 태그 부착
...
`SOURCE_TIERS.md` 기준으로 각 소스에 S/A/B/C 등급을 부여합니다.
**핵심**: arXiv 프리프린트는 최대 A등급. S등급은 공식 문서 + peer-reviewed 학술지만.

등급 판단 시 참고:
- 저자/조직의 전문성, 데이터/출처 명시 여부
- 발행 시점의 최신성, peer review 존재 여부
- 코드 기반 사전분류(`dr-verify classify`)가 있으면 이를 기본값으로 수용
...
#### 발견 1: {제목}
- **소스**: {URL} (등급: {S/A/B/C})
- **내용**: {핵심 1-3문장만. 장문 서술 금지. 수치/패턴/결론만 추출}

#### 발견 2: {제목}
- **소스**: {URL} (등급: {S/A/B/C})
- **내용**: {핵심 1-3문장만}

(소스당 최대 3문장. 출력 토큰을 최소화합니다.)

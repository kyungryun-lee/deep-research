...
## Phase 8: DEPLOY (사용자 확인 후 Git 배포)
...
Git에 반영할까요?
- "반영해줘" / "push 해줘" → 즉시 commit + push (재확인 없음)
- "아니오" → 변경은 로컬에 유지, Git 미반영
```

### 8.2 Git 반영 (사용자가 명시적으로 요청한 경우에만)

사용자가 "반영", "push", "커밋", "배포" 등을 명시하면 **묻지 않고 즉시** 실행:

1. **변경 파일 스테이징**: `git add {변경된 파일들}`
2. **커밋 메시지 생성**:
   ```
   {변경 요약 1줄}
   ...
   Source: deep-research auto-improvement pipeline
...
- **Phase 8 Git 반영은 사용자가 명시적으로 요청한 경우에만 실행합니다**
- **"반영해", "push해" 등 명시적 요청 시에는 재확인 없이 즉시 실행합니다**

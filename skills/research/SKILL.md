...
### Prompt Caching 정렬 원칙 (Anthropic 공식, 5분 TTL)
모든 agent 호출 프롬프트는 **STABLE prefix → VARIABLE suffix** 순서로 작성합니다:
- **STABLE prefix** (캐시 적중 영역): 작성 지시, rubric 파일 내용, 차원명, 토큰 예산 규칙, 처리 규칙
- **VARIABLE suffix** (캐시 미스 영역): query, findings, per-run JSON 결과, iteration 카운터
- **효과**: 동일 rubric으로 5분 내 반복 시 토큰 -50~90% (캐시된 prefix 재사용)
- **주의**: rubric 파일/instruction 텍스트의 공백·JSON 키 순서 변경은 cache invalidation 유발 — 결정론적 직렬화 유지
...
```
==== STABLE PREFIX (prompt cache 적중을 위해 STABLE 내용을 앞에 배치) ====
...
==== VARIABLE SUFFIX (per-run, 캐시 미스 영역) ====
...
### Phase 5.0: Input size gating — 1-pass vs subagent split (코드 — AI 호출 0)

**Why**: Opus 4.7은 1M context를 보유하므로 대부분의 리서치(findings <200K 토큰)는 단일 Synthesizer 호출로 끝낼 수 있습니다. Anthropic 공식 multi-agent 권고에 따라, 큰 입력만 subagent로 분기합니다.

```bash
# 1) 대략적 토큰 추정 (Opus 4.7 tokenizer ~1.3 factor)
WORD_COUNT=$(wc -w < {all_findings_file})
TOKEN_ESTIMATE=$(( WORD_COUNT * 13 / 10 ))

# 2) 분기
if [ "$TOKEN_ESTIMATE" -lt 200000 ]; then
    SYNTH_MODE="single-pass"
    # all_findings 원본 그대로 Synthesizer에 전달 (아래 본 호출)
else
    SYNTH_MODE="subagent-split"
    # Worker별 섹션을 병렬 Sonnet subagent로 압축 후 Synthesizer 호출
    # (== Worker {role}: 마커 기준으로 split → 각 섹션을 Sonnet에 200단어 압축 요청 → 합본)
fi

echo "[Phase 5.0] findings ~${TOKEN_ESTIMATE} tokens → ${SYNTH_MODE}"
```

**SYNTH_MODE = "single-pass"** (대부분의 경우):
- Synthesizer 호출 1회, all_findings 원본 전달 → 보고서 일관성 최대, 벽시계 시간 단축

**SYNTH_MODE = "subagent-split"** (findings ≥200K, 드문 케이스):
- Worker 마커(`=== Worker {role}:`) 기준 분할
- 각 섹션을 Sonnet subagent로 200단어 압축 (병렬, `&` + `wait`)
- 압축 결과를 합쳐 `all_findings_compressed`로 Synthesizer에 전달
- ⚠️ 압축은 정보 손실을 유발하므로 200K 미만일 땐 절대 사용하지 않음

### Phase 5.1: Synthesizer 호출

research-synthesizer 에이전트를 호출합니다.

프롬프트:
```
==== STABLE PREFIX (prompt cache 적중) ====
...
인용 검증 결과 처리 규칙:
→ phantom 인용은 보고서에서 제외
→ weak 인용은 hedge 표현 사용

모순 탐지 결과 처리 규칙:
→ high confidence 충돌은 "소스가 X에 대해 불일치" 명시, 양측 관점 모두 기술
→ medium confidence 충돌도 인지하여 단정적 표현 회피
...
<user_query>{query}</user_query>
<findings>{all_findings if single-pass else all_findings_compressed}</findings>
<synth_mode>{SYNTH_MODE}</synth_mode>
...
### Phase 5.2: 토큰 사용량 기록 (코드 — AI 호출 0)
